...
async def backend_detection_picture(request_data):
    ...
    if os.path.exists(picture_abs_save_path) and picture_relative_save_path != "":
        # todo-如果存在直接调用dll动态库对图片进行推理并返回标注结果数组
        model = init_model()
        # 创建保存tmp对应的图片集合
        ...
        model_cfg = 'detection_process/configs/fcos_r50_caffe_fpn_gn-head_1x_spdfk_coco_3000_2000.py'
        backend_files = ['detection_process/onnxs/fcos_end2end.onnx']
        device = 'cpu'
        result_list = inference_model(model_cfg, deploy_cfg, backend_files, detection_list, device)
        # ------------------------------------------------------use onnx backend do inference-------------------------------------------------------
